...
// Notifies shared code that a m_Window has lost or received input focus
//
...
// Notifies shared code that a m_Window has moved
...
// Notifies shared code that a m_Window has been resized
...
// Notifies shared code that a m_Window has been iconified or restored
//
...
// Notifies shared code that a m_Window has been maximized or restored
//
...
// Notifies shared code that a m_Window framebuffer has been resized
...
// Notifies shared code that a m_Window content scale has changed
...
// Notifies shared code that the m_Window contents needs updating
//
...
// Notifies shared code that the user wishes to close a m_Window
//
...
// Notifies shared code that a m_Window has changed its desired monitor
//
...
void glfwDefaultWindowHints(void)
{
    ...
    // The default is a focused, visible, resizable m_Window with decorations
...
GLFWAPI void glfwDestroyWindow(GLFWwindow* handle)
{
    ...
    // Clear all callbacks to avoid exposing a half torn-down m_Window object
    memset(&window->callbacks, 0, sizeof(window->callbacks));

    // The m_Window's context must not be current on another thread when the
    // m_Window is destroyed
    if (window == _glfwPlatformGetTls(&_glfw.contextSlot))
        glfwMakeContextCurrent(NULL);

    _glfw.platform.destroyWindow(window);

    // Unlink m_Window from global linked list
    {
