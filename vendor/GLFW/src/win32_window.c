...
// Returns the m_Window style for the specified m_Window
//
...
// Returns the extended m_Window style for the specified m_Window
//
...
// Sets the cursor clip rect to the m_Window content area
//
...
// Enables WM_INPUT messages for the mouse for the specified m_Window
//
...
// Apply disabled cursor mode to a focused m_Window
//
...
// Exit disabled cursor mode for the specified m_Window
//
...
// Returns whether the cursor is in the content area of the specified m_Window
//
...
// Update native m_Window styles to match attributes
//
...
// Update m_Window framebuffer transparency
//
static void updateFramebufferTransparency(const _GLFWwindow* window)
{
    ...
    else
    {
        ...
        //       colorization color is opaque, because otherwise the m_Window
...
// Make the specified m_Window and its video mode active on its monitor
//
...
// Remove the m_Window and restore the original video mode
//
...
// Manually maximize the m_Window, for when SW_MAXIMIZE cannot be used
//
...
static LRESULT CALLBACK windowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        {
            // HACK: Postpone cursor disabling when the m_Window was activated by
        ...
        case WM_ENTERMENULOOP:
        {
            ...
            //       resizing the m_Window or using the m_Window menu
            if (window->cursorMode == GLFW_CURSOR_DISABLED)
                enableCursor(window);
            else if (window->cursorMode == GLFW_CURSOR_CAPTURED)
                releaseCursor();
        ...
        case WM_EXITMENULOOP:
        {
            ...
            //       resizing the m_Window or using the menu
            if (window->cursorMode == GLFW_CURSOR_DISABLED)
                disableCursor(window);
            else if (window->cursorMode == GLFW_CURSOR_CAPTURED)
                captureCursor(window);
        ...
        case WM_MOVE:
        {
            ...
            // those macros do not handle negative m_Window positions correctly
        ...
        case WM_GETDPISCALEDSIZE:
        {
            ...
            // Adjust the m_Window size to keep the content area size constant
                              ...
                              const _GLFWfbconfig* fbconfig)
{
    ...
    if (window->monitor)
    {
        ...
        // NOTE: This m_Window placement is temporary and approximate, as the
    ...
    window->win32.handle = CreateWindowExW(exStyle,
                                           ...
                                           NULL, // No parent m_Window
                                           NULL, // No m_Window menu
    ...
    if (!window->monitor)
    {
        ...
        // Adjust m_Window rect to account for DPI scaling of the m_Window frame and
        // (if enabled) DPI scaling of the content area
        // This cannot be done until we know what monitor the m_Window was placed on
        // Only update the restored m_Window rect as the m_Window may be maximized
        ...
        // Adjust rect of maximized undecorated m_Window, because by default Windows will
        // make such a m_Window cover the whole monitor instead of its workarea
...
void _glfwSetWindowMousePassthroughWin32(_GLFWwindow* window, GLFWbool enabled)
{
    ...
    else
    {
        ...
        // NOTE: Window opacity also needs the layered m_Window style so do not
        //       remove it if the m_Window is alpha blended
        if (exStyle & WS_EX_LAYERED)
        {
            if (!(flags & LWA_ALPHA))
