    ...
    for (const Position &movePattern : m_MovePatterns) {
        ...
        if (
            (
                ...
                || m_OwnerBoard->IsPieceCapturable(m_Position + movePattern,
                                                   m_Color)
            )
            &&
...
bool King::Move(Position pos, bool overrideLegality/* =false */) {
    ...
    if (Piece::Move(pos, overrideLegality)) {
            ...
            (prev - pos).file < -1) // if king just castled
        {
            ...
            m_OwnerBoard->MakeMove(rook, rookPos,
...
void Pawn::CalculateLegalMoves() {
    ...
    if (
        ...
        if (m_OwnerBoard->IsPieceCapturable(m_Position + m_MovePatterns[2],
                                            m_Color))
    ...
    if (
        ...
        if (m_OwnerBoard->IsPieceCapturable(m_Position + m_MovePatterns[3],
                                            m_Color))
...
bool Pawn::Move(Position pos, bool overrideLegality) {
    ...
    if (Piece::Move(pos, overrideLegality)) {
        ...
        if (abs((pos - prev).rank) > 1) // If pawn moved two squares
        {
            ...
            m_OwnerBoard->SetPiece(enPassantPos,
...
LegalMoveSprite::LegalMoveSprite(float squareSize, float spriteSize,
    ...
    m_Obj = Engine::Renderer::GenQuad(viewPos, spriteSize,
    ...
    m_Obj.shader.SetUniformVec(m_Obj.shader.GetUniformLocation("tint"), 4,
                               tint);
}
...
void LegalMoveSprite::SetPosition(Position pos) {
    ...
    m_Obj.shader.SetUniformVec(m_Obj.shader.GetUniformLocation("renderOffset"),
                               2, viewPos);
}
