...
SlidingPiece::SlidingPiece(Color color, Position pos, float squareSize, char *pieceName,
                           Board *board)
...
void SlidingPiece::CalculateLegalMoves() {
    ...
    for (const Position &movePattern: m_MovePatterns) {
        ...
        // only allow the piece to move so that the rules are not broken
        if (!m_PinnedDirections.empty() &&
            (m_PinnedDirections.find(movePattern) == m_PinnedDirections.end() ||
             m_PinnedDirections.find(-movePattern) == m_PinnedDirections.end()))
            continue;
        ...
        bool isEnemyPiece = m_OwnerBoard->IsPieceCapturable(latest + movePattern, m_Color) &&
...
bool SlidingPiece::FindPinnedPiece(Position curPos, Position movePattern) {
    ...
    while ((curPos + movePattern).IsValid()) {
        if (m_OwnerBoard->IsSquareOccupied(curPos + movePattern)) {
            if (m_OwnerBoard->GetPiece(curPos + movePattern)->GetPieceName() == "king") {
                pinned = true;
                ...
                if (!isAlreadyPinned) {
                    if (m_PinnedPiecePos.IsValid()) {
                        std::cout << "unpinned: " << m_PinnedPiecePos.ToString() << std::endl;
                        Piece *prevPinnedPiece = m_OwnerBoard->GetPiece(m_PinnedPiecePos);
                        prevPinnedPiece->UnPin(movePattern);
