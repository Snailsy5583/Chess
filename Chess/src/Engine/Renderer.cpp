	...
	RendererObject& Renderer::GenQuad(const float pos[3], const float sideLen, const char* vertShaderPath, const char* fragShaderPath)
	{
		...
		for (int row = 0; row < 6; row++)
		{
			for (int col = 0; col < 3; col++)
				verts[(row*5)+col] = (m_QuadVerts[(row*5)+col]*sideLen) + pos[col];
			for (int col = 3; col < 5; col++)
				verts[(row*5)+col] = m_QuadVerts[(row*5)+col];
		}

		return GenObject(pos, sizeof(verts), verts, vertShaderPath, fragShaderPath);
	}

	void Renderer::MoveQuad(RendererObject& obj, const float newPos[3], float sideLen)
	{
		for (int i = 0; i < 3; i++)
		...
		{
			for (int col = 0; col < 3; col++)
				verts[(row * 5) + col] = (m_QuadVerts[(row * 5) + col] * sideLen) + newPos[col];
			for (int col = 3; col < 5; col++)
				verts[(row * 5) + col] = m_QuadVerts[(row * 5) + col];
		}
